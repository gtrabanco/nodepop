...
module.exports = function (req, res, next) {
    ...
    var error = {status: 403, message: "Forbidden access", error: "The site is only accesible throught the https protocol."};
    //res.redirect('https://' + req.hostname + ':' + req.app.get('secure port') + req.originalUrl);
    if (req.originalUrl.match(/\/apiv[0-9]*/i)) {
        console.info('Returning an error for the api.');
        res.status(403).json(error);
    }  else {
        console.info('Returning an error for the site.');
        res.status(403);
